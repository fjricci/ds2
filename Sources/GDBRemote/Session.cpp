...
void Session::Handle_jThreadsInfo(ProtocolInterpreter::Handler const &,
                                  std::string const &) {
  StopCode stop;
  ErrorCode error = _delegate->onQueryThreadStopInfo(*this, ProcessThreadId(), true, stop);
  if (error != kSuccess) {
    sendError(error);
    return;
  }
  ...
  for (auto &tid : stop.threads) {
    _delegate->onQueryThreadStopInfo(*this, ProcessThreadId(kAnyProcessId, tid), true, stop);
